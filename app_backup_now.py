...
class BlackjackGame(db.Model):
    ...
    winnings = db.Column(db.Float, nullable=False)
    timestamp = db.Column(db.DateTime, default=datetime.utcnow)
    user = db.relationship('User', backref='blackjack_games')
...
def migrate_database():
    try:
        with db.engine.connect() as conn:
            ...
            result = conn.execute(db.text("SHOW TABLES LIKE 'transactions'"))
            if not result.fetchone():
                ...
                conn.execute(db.text("""
                    CREATE TABLE transactions (
                        ...
                        type ENUM('deposit', 'withdrawal', 'referral', 'bonus', 'game_win', 'game_loss', 'admin_adjustment') NOT NULL,
                ...
                conn.execute(db.text("CREATE INDEX idx_transactions_user_id ON transactions(user_id)"))
                conn.execute(db.text("CREATE INDEX idx_transactions_type ON transactions(type)"))
                conn.execute(db.text("CREATE INDEX idx_transactions_status ON transactions(status)"))
...
def admin_game_activity():
    ...
    # Fetch recent game activity based on the selected game type
    try:
        conn = get_db_connection()
        cursor = conn.cursor(dictionary=True)
        ...
        if game_type == 'all':
            ...
            cursor.execute("""
                (SELECT 'Mines' as game_type, user_id, bet_amount, result, timestamp 
                FROM mines_game ORDER BY timestamp DESC LIMIT %s)
                UNION ALL
                (SELECT 'Plinko' as game_type, user_id, bet_amount, 
                    CASE WHEN winnings > 0 THEN 'win' ELSE 'lose' END as result, 
                    timestamp 
                FROM plinko_game ORDER BY timestamp DESC LIMIT %s)
                UNION ALL
                (SELECT 'Slots' as game_type, user_id, bet_amount,
                    CASE WHEN winnings > 0 THEN 'win' ELSE 'lose' END as result,
                    timestamp 
                FROM slot_game ORDER BY timestamp DESC LIMIT %s)
                UNION ALL
                (SELECT 'CoinFlip' as game_type, user_id, bet_amount, outcome as result, timestamp 
                FROM coin_flip_game ORDER BY timestamp DESC LIMIT %s)
            ...
            if table:
                # Adjust query based on table structure (simplified example)
                cursor.execute(f"""
    ...
    return render_template('admin_game_activity.html', 
                          ...
                          game_types=['all', 'Mines', 'Plinko', 'Slots', 'MegaSlots', 
                                     'LuckyWheel', 'Aviator', 'CoinFlip', 'OddEven'])
...
@login_required
def blackjack():
    return render_template('blackjack.html', balance=current_user.wallet_balance)
...
def blackjack_start():
    try:
        ...
        dealer_score = calculate_blackjack_score([dealer_cards[0]])  # Only show first card initially
        
        # Check for natural blackjack
        player_blackjack = player_score == 21
        dealer_blackjack = calculate_blackjack_score(dealer_cards) == 21
        
        # Determine result for natural blackjack
        if player_blackjack and dealer_blackjack:
            ...
            result = 'win'
            multiplier = 2.5  # Blackjack typically pays 3:2
            winnings = bet_amount * multiplier
            ...
            return jsonify({
                'success': True,
                'player_cards': player_cards,
                'dealer_cards': [dealer_cards[0], '??'],  # Hide second dealer card
        ...
        # If we get here, game ended with natural blackjack
        deduct_bet_amount(current_user, bet_amount)
        if winnings > 0:
            add_winnings(current_user, winnings)
        
        # Save game
        save_blackjack_game(current_user.id, bet_amount, player_cards, dealer_cards, 
                           player_score, calculate_blackjack_score(dealer_cards), 
                           result, multiplier, winnings)
        ...
        db.session.rollback()
        print(f"Error in blackjack_start: {str(e)}")
        return jsonify({'success': False, 'error': 'An error occurred'})
...
def blackjack_hit():
    try:
        ...
        # Calculate new score
        player_score = calculate_blackjack_score(game['player_cards'])
        ...
        if player_score > 21:
            ...
            # Save game and update balance
            deduct_bet_amount(current_user, game['bet_amount'])
            
            save_blackjack_game(current_user.id, game['bet_amount'], game['player_cards'], 
                              game['dealer_cards'], player_score, 
                              calculate_blackjack_score(game['dealer_cards']), 
                              result, multiplier, winnings)
            ...
            return jsonify({
                ...
                'player_score': player_score,
                'dealer_score': calculate_blackjack_score(game['dealer_cards']),
                'result': 'bust',
                'game_over': True,
                'new_balance': current_user.wallet_balance
            })
        ...
        return jsonify({
            'success': True,
            'player_cards': game['player_cards'],
            'dealer_cards': [game['dealer_cards'][0], '??'],  # Still hide dealer's second card
            'player_score': player_score,
            'dealer_score': calculate_blackjack_score([game['dealer_cards'][0]]),
            'can_hit': True,
            'can_stand': True,
            'game_over': False
        })
        ...
        print(f"Error in blackjack_hit: {str(e)}")
        return jsonify({'success': False, 'error': 'An error occurred'})
...
def blackjack_stand():
    try:
        ...
        # Dealer plays - dealer must hit until reaching 17 or higher
        dealer_score = calculate_blackjack_score(game['dealer_cards'])
        ...
        # Save game
        save_blackjack_game(current_user.id, game['bet_amount'], game['player_cards'], 
                          game['dealer_cards'], player_score, dealer_score,
                          result, multiplier, winnings)
        ...
        print(f"Error in blackjack_stand: {str(e)}")
        return jsonify({'success': False, 'error': 'An error occurred'})

@app.route('/blackjack/history')
@login_required
def blackjack_history():
    try:
        # Get user's recent games
        games = BlackjackGame.query.filter_by(
            user_id=current_user.id
        ).order_by(BlackjackGame.timestamp.desc()).limit(20).all()
        
        history = []
        for game in games:
            history.append({
                'bet_amount': game.bet_amount,
                'player_cards': game.player_cards.split(','),
                'dealer_cards': game.dealer_cards.split(','),
        ...
        print(f"Error in blackjack_history: {str(e)}")
        return jsonify({'success': False, 'error': 'An error occurred'})
...
def save_blackjack_game(user_id, bet_amount, player_cards, dealer_cards, player_score, dealer_score, result, multiplier, winnings):
    """Save the blackjack game to the database"""
    game = BlackjackGame(
        user_id=user_id,
        bet_amount=bet_amount,
        player_cards=','.join(player_cards),
        dealer_cards=','.join(dealer_cards),
        player_score=player_score,
        dealer_score=dealer_score,
        result=result,
        multiplier=multiplier,
...
def deduct_bet_amount(user, amount):
    """Deduct bet amount from user's wallet"""
    user.wallet_balance -= amount
    db.session.commit()

def add_winnings(user, amount):
